...
class Queue:
    ...
    def add_link_pending(links, seed=None):
        for link in links:
            if not exist_pending_link(link):
                save_pending_link([link, 'false', str(datetime.now()), seed])

    @staticmethod
    def next():
        ID = 0
        URI = 1
        link_data = next_pending_link()
        if link_data[ID] == -1:
            return None
        else:
            return {
    ...
    def add_crawled_link(onion_link_data, link_pending):
        save_crawled_link(onion_link_data)
        check_link(link_pending)
    ...
    def is_parent_offline(uri):
        ...
        result = is_offline(parent_domain)
        if result:
            return {
                'id': result[PARENT_ID],
